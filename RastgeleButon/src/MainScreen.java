...
///////////////////////////////////////////////////////////////////////////////////Sinema Sütun Özellikleri
		...
		/************************* Şimdilik Veri Ekliyorum ***************************/
		/*
		 * cinemaData[0] = "Recep İvedik 7"; cinemaData[1] = "Komedi"; cinemaData[2] =
		 * "Şahan Gökbakar"; cinemaData[3] = 1; cinemaData[4] = "13:40";
		 * cinemaModel.addRow(cinemaData); cinemaData[0] = "Hızlı ve Öfkeli 10";
...
///////////////////////////////////////////////////////////////////////////////////Tiyatro Sütun Özellikleri
		...
		comboBox_Method.addItem("Banka/Kredi Kartı");
		...
		comboBox_Method.addItemListener(new ItemChangeListener() {
			public void itemStateChanged(ItemEvent arg0) {
				...
				if (comboBox_Method.getSelectedIndex() == 0) {
					w_pane_Kart.setVisible(false);
					// BuyTicket.setlblCard(BuyTicket.price+" ödemeniz vardır.");
				} else {
					w_pane_Kart.setVisible(true);
					// BuyTicket.setlblCard("Ücret, BANKA/KREDİ KARTI ile ödenmiştir. Borcunuz
					// yoktur.");
				}
			}
		});
		...
		btn_BuyTicket.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (comboBox_Method.getSelectedIndex() == 1) {
					BuyTicket.setlblCard("Ücret, BANKA/KREDİ KARTI ile ödenmiştir. Borcunuz yoktur.");
					BuyTicket.setPrice("Ödendi");
					...
					if (fld_MovieName.getText().length() == 0 || fld_MovieType.getText().length() == 0
						...
						JOptionPane.showMessageDialog(null, "Gerekli birkaç bilgi eksik.", "Mesaj",
					...
					if (fld_CartName.getText().length() == 0 || fld_CartNumber.getText().length() ==0
						...
						JOptionPane.showMessageDialog(null, "Kart bilgilerinde hatalı giriş.", "Mesaj",
						...
						if (ticket.comboBox_seat.getItemCount() == 0) {
							JOptionPane.showMessageDialog(null, "Lütfen Koltuk Seçiniz.", "Mesaj",
									JOptionPane.INFORMATION_MESSAGE);

						}
						else {
							if (JOptionPane.showConfirmDialog(null, "Satın Almayı Onaylıyor musunuz?", "Dikkat!",
									JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
							
								Metod_Helper.showMsg("Ödemeniz Alınıyor. Lütfen Bekleyiniz...");
							try {
							...
							Metod_Helper.showMsg("Ödemeniz Başarıyla Gerçekleşti");
					...
					BuyTicket.setlblCard(BuyTicket.price + " ödemeniz vardır. İyi günler Dileriz.");
					if (fld_MovieName.getText().length() == 0 || fld_MovieType.getText().length() == 0
							|| fld_Director.getText().length() == 0 || fld_Salon.getText().length() == 0
							|| fld_Seance.getText().length() == 0 || fld_Name_cinema.getText().length() == 0
							|| fld_Surname_cinema.getText().length() == 0 || fld_Mail_cinema.getText().length() == 0) {
		...
		JLabel lbl_GameName = new JLabel("Oyun Adı:");
		...
		comboBox_Method3.addItem("Banka/Kredi Kartı");
			...
			public void actionPerformed(ActionEvent e) {
				if (tabbedPane.getSelectedIndex() == 0) {
					...
					if (selRow >= 0) {
						...
						lbl_Poster.setText("Poster");// Poster, Film Kayıt işleminde poster ekledikten sonra
														// ayarlanılacak
				...
				if (tabbedPane.getSelectedIndex() == 1) {
					...
					if (selRow1 >= 0) {
						...
						lbl_Poster2.setText("Poster1");// Poster, Film Kayıt işleminde poster ekledikten sonra
														// ayarlanılacak
